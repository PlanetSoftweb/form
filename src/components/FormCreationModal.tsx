...
import { X, PenLine, Sparkles, Layout, ArrowRight, Loader2, AlertCircle, RefreshCw, Mail } from 'lucide-react';
import { generateFormWithGemini } from '../lib/geminiService';
import toast from 'react-hot-toast';
import { AirportLoadingAnimation } from './AirportLoadingAnimation';
...
export const FormCreationModal = ({ isOpen, onClose }: Props) => {
  ...
  const handleAIForm = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const generatedForm = await generateFormWithGemini(aiPrompt);
      const formId = await saveForm({
        ...generatedForm,
        userId: user!.uid,
      });
      ...
      const errorMessage = error?.message || 'Failed to generate form with AI. Please try again.';
  ...
  const handleContactSupport = () => {
    window.open('mailto:support@formbuilder.com?subject=AI Form Generation Error&body=I encountered an error while generating a form with AI.', '_blank');
  };
  ...
  return (
    <>
      <AirportLoadingAnimation isVisible={loading && step === 'ai'} />
      <AnimatePresence>
        {isOpen && (
          <motion.div
          initial={{ opacity: 0 }}
